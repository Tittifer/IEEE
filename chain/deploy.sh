...
NETWORK_DIR=${SCRIPT_DIR}/../chain_docker
...
function installChaincode() {
  ...
  # 复制链码包到CLI容器
  docker cp ${CHAINCODE_DIR}/${CHAINCODE_NAME}.tar.gz cli_chain:/opt/gopath/src/github.com/hyperledger/fabric/peer/${CHAINCODE_NAME}.tar.gz
  ...
  # 安装链码到peer0.org1
  docker exec cli_chain peer lifecycle chaincode install ${CHAINCODE_NAME}.tar.gz
  ...
  # 安装链码到peer1.org1
  docker exec -e CORE_PEER_ADDRESS=peer1.org1.chain.com:8061 \
          -e CORE_PEER_TLS_CERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org1.chain.com/peers/peer1.org1.chain.com/tls/server.crt \
          -e CORE_PEER_TLS_KEY_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org1.chain.com/peers/peer1.org1.chain.com/tls/server.key \
          -e CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org1.chain.com/peers/peer1.org1.chain.com/tls/ca.crt \
          cli_chain peer lifecycle chaincode install ${CHAINCODE_NAME}.tar.gz
...
function getInstalledPackageID() {
  ...
  PACKAGE_ID=$(docker exec cli_chain peer lifecycle chaincode queryinstalled | grep ${CHAINCODE_LABEL} | awk '{print $3}' | sed 's/,$//')
...
function approveChaincode() {
  ...
  # Org1批准链码
  docker exec cli_chain peer lifecycle chaincode approveformyorg \
...
function commitChaincode() {
  ...
  # 检查批准状态
  docker exec cli_chain peer lifecycle chaincode checkcommitreadiness \
    --channelID ${CHANNEL_NAME} \
    --name ${CHAINCODE_NAME} \
    --version ${CHAINCODE_VERSION} \
    --sequence ${CHAINCODE_SEQUENCE} \
  ...
  # 提交链码定义
  docker exec cli_chain peer lifecycle chaincode commit \
    -o orderer.chain.com:8050 \
    --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/chain.com/orderers/orderer.chain.com/msp/tlscacerts/tlsca.chain.com-cert.pem \
    --channelID ${CHANNEL_NAME} \
    --name ${CHAINCODE_NAME} \
...
function generateCLIScript() {
  ...
  # 复制模板脚本到chain_docker目录
  cp ${NETWORK_DIR}/chain_cli.sh ${NETWORK_DIR}/chain_cli.sh.bak
  
  # 更新脚本中的注释
  sed -i "s/# 由deploy.sh自动生成/# 由deploy.sh自动生成 - $(date)/" ${NETWORK_DIR}/chain_cli.sh
  
  # 确保脚本有执行权限
  chmod +x ${NETWORK_DIR}/chain_cli.sh
