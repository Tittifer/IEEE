...
function networkUp() {
  ...
  # 如果网络已经在运行，先关闭
  if [ $(docker ps -q --filter name=peer0.org1.mainchain.com | wc -l) -gt 0 ]; then
...
function createChannel() {
  ...
  # peer0.org1加入通道
  docker exec cli_mainchain peer channel join -b mainchannel.block
  ...
  docker exec -e CORE_PEER_ADDRESS=peer1.org1.mainchain.com:8061 \
              ...
              cli_mainchain peer channel join -b mainchannel.block
