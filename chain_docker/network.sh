...
function generateGenesisBlock() {
  ...
  # 生成应用通道交易
  configtxgen -profile TwoOrgsChannel -outputCreateChannelTx ${ROOTDIR}/channel-artifacts/channel.tx -channelID mainchannel
  ...
  # 生成锚节点更新交易
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ${ROOTDIR}/channel-artifacts/Org1MSPanchors.tx -channelID mainchannel -asOrg Org1MSP
...
function networkUp() {
  ...
  # 如果网络已经在运行，先关闭
  if [ $(docker ps -q --filter name=peer0.org1.chain.com | wc -l) -gt 0 ]; then
...
function createChannel() {
  ...
  # 进入CLI容器创建通道
  docker exec cli_chain peer channel create -o orderer.chain.com:8050 -c mainchannel -f /opt/gopath/src/github.com/hyperledger/fabric/peer/channel-artifacts/channel.tx --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/chain.com/orderers/orderer.chain.com/msp/tlscacerts/tlsca.chain.com-cert.pem
  ...
  # peer0.org1加入通道
  docker exec cli_chain peer channel join -b mainchannel.block
  ...
  docker exec -e CORE_PEER_ADDRESS=peer1.org1.chain.com:8061 \
              ...
              cli_chain peer channel join -b mainchannel.block
